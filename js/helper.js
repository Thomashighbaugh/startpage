var helper = (function() {
    ...
    if (string.indexOf("|") > 0 && string.indexOf("|") < string.length - 1) {
      attributes.forEach(function(arrayItem, index) {
        if (arrayItem.indexOf(":") > 0) {
          // if key and value
          var pair =
            arrayItem.substring(0, arrayItem.indexOf(":")) +
            "," +
            arrayItem.substring(arrayItem.indexOf(":") + 1, arrayItem.length);
          pair = pair.split(",");
          attributes[index] = {
            key: pair[0],
            value: pair[1]
      ...
      attributes.forEach(function(arrayItem, index) {
        if (
          "key" in arrayItem &&
          arrayItem.key != undefined &&
          ("value" in arrayItem && arrayItem.value != undefined)
        ) {
          element.setAttribute(arrayItem.key, arrayItem.value);
  ...
  var toWords = function(number) {
    ...
    var _generateWords = function(number) {
      ...
      if (number < 20) {
        ...
        word =
          _generateWords(Math.floor(number / oneQuadrillion)) + " Quadrillion,";
      }
      words.push(word);
  ...
  var ordinalWords = function(words) {
    ...
    if (
      endsWithDoubleZeroPattern.test(words) ||
