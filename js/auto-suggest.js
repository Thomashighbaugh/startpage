var autoSuggest = (function() {
  ...
  var _navigateResults = function(event) {
    ...
    var _keyEvents = function() {
      ...
      if (
        event.keyCode == 39 &&
        document.activeElement != _currentInputOptions.input
      ) {
        event.preventDefault();
        if (focusIndex == null) {
          elementToFocus = allSuggestItems[0];
  ...
  var _checkClick = function(event) {
    if (
      !event.target.classList.contains("auto-suggest-list") &&
      !event.target.classList.contains("auto-suggest-input")
    ) {
      destroy();
    }
  };
  ...
  var _getSuggestItems = function() {
    var searchTerm = _currentInputOptions.input.value
      .replace(/^\s+/, "")
      .replace(/\s+$/, "")
      .toLowerCase();
    var action = {
      ...
      var action = {
        fontawesomeIcon: function() {
          suggestItems.forEach(function(arrayItem) {
            ...
            var anchor = helper.node(
              "a|href:#,tabindex:1,class:auto-suggest-link"
            );
            var icon = helper.node(
              "span|class:auto-suggest-icon fa-" + arrayItem.name
            );
            ...
            var text = helper.node(
              "span:" + arrayItem.label + "|class:auto-suggest-icon-text"
            );
    ...
    var _render_autoSuggestList = function() {
      ...
      if (autoSuggestList) {
        ...
        var style = {
          ...
          top:
            autoSuggestWrapper.getBoundingClientRect().bottom + window.scrollY,
          width: autoSuggestWrapper.getBoundingClientRect().width
        };
        var autoSuggestList = helper.node(
          "ul|class:auto-suggest-list list-unstyled"
        );
        body.appendChild(autoSuggestList);
        autoSuggestList.setAttribute(
          "style",
          "width: " +
            style.width +
            "px; top: " +
